...
final class DefaultStateData: CustomApplicationStateData {
    ...
    init(_ app: App) {
        ...
        self.systemLibrary = self.fm.homeDirectoryForCurrentUser.appendingPathComponent("Library/Saved Application State")
    ...
    func copy() throws {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + .milliseconds(500)) { //[unowned self] in
            _ = try? StateDataUtils.createDirectory(at: self.profileState)
